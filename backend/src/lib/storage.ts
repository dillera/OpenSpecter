/**
 * Cloudflare R2 storage utilities for Open Specter document management.
 ...
 *   R2_BUCKET_NAME      — bucket name (default: "open-specter")
 */
...
const BUCKET = process.env.R2_BUCKET_NAME ?? "open-specter";
...
const LOCAL_STORAGE_ROOT =
  process.env.LOCAL_STORAGE_ROOT ?? "/app/data/open-specter-storage";

function localFilePath(key: string): string {
  const normalized = path.normalize(key).replace(/^([.][.][\/\\])+/, "");
  const fullPath = path.join(LOCAL_STORAGE_ROOT, normalized);
  const rootWithSep = path.resolve(LOCAL_STORAGE_ROOT) + path.sep;
...
export async function downloadFile(key: string): Promise<ArrayBuffer | null> {
  if (!storageEnabled) {
    try {
      const bytes = await readFile(localFilePath(key));
...
export async function deleteFile(key: string): Promise<void> {
  if (!storageEnabled) {
    await unlink(localFilePath(key)).catch(() => {});
    return;
  }
  const client = getClient();
