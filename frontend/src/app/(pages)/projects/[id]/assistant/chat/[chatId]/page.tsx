    ...
    return (
        <div className="flex-1 flex items-center justify-center">
            <div className="relative flex items-center justify-center h-[30px]">
                ...
                >
                    <OpenSpecterIcon size={ICON_SIZE} />
                </div>
                <h1
                    ref={textRef}
...
function Divider({ onDrag }: { onDrag: (dx: number) => void }) {
    ...
    return (
        <div className="relative w-0 shrink-0 z-10">
            ...
            >
                {isDragging && (
                    <div className="w-1 bg-foreground transition-colors" />
                )}
            </div>
        </div>
    );
...
export default function ProjectAssistantChatPage({ params }: Props) {
    ...
    const [initialMessages] = useState<OpenSpecterMessage[]>(newChatMessages ?? []);
    ...
    const handleDocClick = (doc: OpenSpecterDocument) => {
        openTab(doc.id, doc.filename);
    };

    const handleCitationClick = (citation: OpenSpecterCitationAnnotation) => {
        openTab(
    ...
    const handleEditViewClick = (ann: OpenSpecterEditAnnotation, filename: string) => {
    ...
    const handleChatDrop = (e: React.DragEvent) => {
        e.preventDefault();
        const docId = e.dataTransfer.getData("application/open-specter-doc");
        if (!docId) return;
            ...
            <div className="flex items-center justify-between px-8 py-4 shrink-0">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        title="Delete chat"
                        className="flex items-center justify-center p-1.5 text-gray-500 hover:text-foreground transition-colors disabled:opacity-40"
                    >
                        {deletingChat ? (
                            <Loader2 className="h-4 w-4 animate-spin" />
                        ) : (
                ...
                {!explorerCollapsed && (
                    <>
                            ...
                            onDragOver={(e) => {
                                ...
                                const isInternal =
                                    Array.from(e.dataTransfer.types).includes(
                                        "application/open-specter-doc",
                                    ) ||
                                    Array.from(e.dataTransfer.types).includes(
                                        "application/open-specter-folder",
                                    );
                        ...
                        >
                            ...
                            {/* Drop overlay */}
                            <div
                                className={`flex-1 overflow-y-auto relative h-full ${explorerDragOver ? "bg-muted/50" : ""}`}
                                onDragOver={(e) => {
                                    e.preventDefault();
                                }}
                                onDrop={async (e) => {
