...
import {
    ...
    getProjectPeople,
    type OpenSpecterDocumentVersion,
...
function DocIcon({ fileType }: { fileType: string | null }) {
    if (fileType === "pdf")
        return <FileText className="h-4 w-4 text-foreground shrink-0" />;
...
export function ProjectPage({ projectId }: Props) {
    const [project, setProject] = useState<OpenSpecterProject | null>(null);
    const [folders, setFolders] = useState<OpenSpecterFolder[]>([]);
    const [chats, setChats] = useState<OpenSpecterChat[]>([]);
    ...
    const [uploadVersionDoc, setUploadVersionDoc] =
        useState<OpenSpecterDocument | null>(null);
    const [viewingDoc, setViewingDoc] = useState<OpenSpecterDocument | null>(null);
    ...
    const [versionsByDocId, setVersionsByDocId] = useState<
        Map<string, OpenSpecterDocumentVersion[]>
    >(() => new Map());
     ...
     */
    function handleUploadNewVersion(doc: OpenSpecterDocument) {
        setUploadVersionDoc(doc);
    }

    async function submitNewVersion(
        doc: OpenSpecterDocument,
        file: File,
        displayName: string,
    ) {
        try {
    ...
    useEffect(() => {
        Promise.all([
            ...
            listProjectChats(projectId).catch(() => [] as OpenSpecterChat[]),
    ...
    async function handleCreateFolder(parentId: string | null) {
        ...
        const optimistic: OpenSpecterFolder = {
            id: tempId,
    ...
    function handleDocsSelected(newDocs: OpenSpecterDocument[]) {
        setProject((prev) =>
            prev ? {
                ...prev,
                documents: [
    ...
    function wouldCreateCycle(movingId: string, targetId: string): boolean {
        ...
        let cur: OpenSpecterFolder | undefined = folders.find((f) => f.id === targetId);
        while (cur) {
    ...
    async function handleDropOnFolder(targetFolderId: string | null, dt: DataTransfer) {
        const docId = dt.getData("application/open-specter-doc");
        const subFolderId = dt.getData("application/open-specter-folder");
        if (docId) {
            const doc = (project?.documents ?? []).find((d) => d.id === docId);
                    ...
                    return (
                        <div key={`doc-${doc.id}`}>
                            <div
                                draggable
                                onDragStart={(e) => {
                                    e.dataTransfer.setData("application/open-specter-doc", doc.id);
                    ...
                    return (
                        <div key={`folder-${folder.id}`}>
                            <div
                                draggable
                                onDragStart={(e) => {
                                    e.dataTransfer.setData("application/open-specter-folder", folder.id);
                                    e.dataTransfer.effectAllowed = "move";
                                    e.stopPropagation();
                                }}
                                ...
                                className={`group flex items-center h-10 pr-8 border-b border-gray-50 hover:bg-gray-50 cursor-pointer transition-colors select-none ${dragOverFolderId === folder.id ? "bg-muted/50 ring-1 ring-inset ring-ring/30" : ""}`}
                            >
                                <div className={`sticky left-0 z-[60] ${CHECK_W} p-2 flex items-center justify-center ${dragOverFolderId === folder.id ? "bg-muted/50" : "bg-white"} group-hover:bg-gray-50 self-stretch`}>
                                    {isExpanded
                                        ? <ChevronDown className="h-3.5 w-3.5 text-gray-400 shrink-0" />
                                        : <ChevronRight className="h-3.5 w-3.5 text-gray-400 shrink-0" />
                                    }
                                </div>
                                <div className={`sticky left-8 z-[60] ${NAME_COL_W} p-2 ${dragOverFolderId === folder.id ? "bg-muted/50" : "bg-white"} group-hover:bg-gray-50`}>
                                <div className="flex items-center gap-1.5">
                                    {isExpanded
                    ...
                    <div className="flex-1 flex flex-col min-h-0">
                        ...
                        <div className="flex-1 flex flex-col min-h-0 relative">
                            {dragOverRoot && dragOverFolderId === null && (
                                <div className="absolute inset-0 border-2 border-border pointer-events-none z-20" />
                            )}
                                ...
                                {q ? (
                                    ...
                                    setViewingDoc(doc);
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        {isProcessing ? <Loader2 className="h-4 w-4 animate-spin text-gray-400 shrink-0" /> : isError ? <AlertCircle className="h-4 w-4 text-muted-foreground shrink-0" /> : <DocIcon fileType={doc.file_type} />}
                                                        <span className="text-sm text-gray-800 truncate">{doc.filename}</span>
                                                    </div>
                                                    </div>
                            ...
                            >
                                ...
                                {contextMenu.showFolderActions && contextMenu.folderId && (
                                    <>
                                        ...
                                        <button
                                            className="w-full px-3 py-1.5 text-left text-foreground hover:bg-muted/50"
                                            onClick={() => {
