...
import type { ColumnConfig, OpenSpecterDocument, TabularCell } from "../shared/types";
            ...
            {documents.map((doc, docIdx) => {
                ...
                return (
                    ...
                    >
                        ...
                        <div
                            className={`sticky left-8 z-[60] ${COL_W} border-b border-r border-gray-200 p-2 text-xs text-gray-800 flex ${expanded ? "items-start" : "items-center"} ${rowBg}`}
                        >
                            <span className={expanded ? "whitespace-normal break-words leading-5" : "line-clamp-1"} title={doc.filename}>
                                {doc.filename}
                            </span>
                        </div>
                        {columns.map((col) => {
                            ...
                            return (
                                <div
                                    key={col.index}
                                    className={`${COL_W} border-b border-r border-gray-200 transition-colors ${isHighlighted ? "bg-muted" : ""}`}
                                >
                                    {cell && (
