...
import type {
    ...
    OpenSpecterEditAnnotation,
} from "../shared/types";
import { EditCard, applyOptimisticResolution } from "./EditCard";
import { PreResponseWrapper } from "../shared/PreResponseWrapper";
import { ThinkingText } from "../shared/ThinkingText";
...
function BulkEditActions({
    ...
    pending: {
        annotation: OpenSpecterEditAnnotation;
        filename: string;
    }[];
    filenameByDocId: Map<string, string>;
    onViewClick?: (ann: OpenSpecterEditAnnotation, filename: string) => void;
    onResolveStart?: (args: {
        editId: string;
        documentId: string;
        verb: "accept" | "reject";
...
function ResponseStatus({ status }: { status: StatusState }) {
    ...
    useEffect(() => {
        if (wasActiveRef.current && !isActive) {
            setShowDone(true);
            setDoneVisible(true);
            const t = setTimeout(() => setDoneVisible(false), 900);
    ...
    if (isError || (showDone && doneVisible)) {
        return (
            <div className="mb-2 flex h-9 w-full items-center">
                <OpenSpecterIcon
                    done={showDone && doneVisible}
                    error={isError}
                    openSpecter={!isError && !(showDone && doneVisible)}
                    size={22}
                />
            </div>
        );
...
function ReasoningBlock({
    ...
    return (
        <div className="relative">
            ...
            >
                {isStreaming ? (
                    <ThinkingText testId="assistant-reasoning-thinking-indicator" />
                ) : (
                    <>
...
function DocReadBlock({
    ...
    return (
        <div className="flex items-start text-sm font-serif text-gray-500 relative">
            ...
            {isStreaming ? (
                <ThinkingText
                    testId="assistant-doc-read-thinking-indicator"
                    className="mt-1.5"
                />
            ) : (
                <div className="mt-2 w-1.5 h-1.5 rounded-full bg-foreground/70 shrink-0" />
            )}
...
function DocFindBlock({
    ...
    return (
        <div className="flex items-start text-sm font-serif text-gray-500 relative">
            ...
            {isStreaming ? (
                <ThinkingText
                    testId="assistant-doc-find-thinking-indicator"
                    className="mt-1.5"
                />
            ) : (
                <div
                    className={`mt-2 w-1.5 h-1.5 rounded-full shrink-0 ${totalMatches > 0 ? "bg-foreground/70" : "bg-gray-300"}`}
                />
            )}
            <div className="ml-2 min-w-0 flex-1 whitespace-normal break-words">
...
function DocCreatedBlock({
    ...
    return (
        <div className="flex items-start text-sm font-serif text-gray-500 relative">
            ...
            {isStreaming ? (
                <ThinkingText
                    testId="assistant-doc-create-thinking-indicator"
                    className="mt-1.5"
                />
            ) : (
                <div className="mt-2 w-1.5 h-1.5 rounded-full bg-foreground/70 shrink-0" />
...
function DocReplicatedBlock({
    ...
    return (
        <div className="flex items-start text-sm font-serif text-gray-500 relative">
            ...
            {isStreaming ? (
                <ThinkingText
                    testId="assistant-doc-replicate-thinking-indicator"
                    className="mt-1.5"
                />
            ) : (
                <div
                    className={`mt-2 w-1.5 h-1.5 rounded-full shrink-0 ${hasError ? "bg-foreground/70" : "bg-foreground/70"}`}
                />
            )}
            <div className="ml-2 min-w-0 flex-1 whitespace-normal break-words">
                <span className="font-medium">{label}</span>{" "}
...
function DocEditedBlock({
    ...
    return (
        <div className="flex items-start text-sm font-serif text-gray-500 relative">
            ...
            {isStreaming ? (
                <ThinkingText
                    testId="assistant-doc-edit-thinking-indicator"
                    className="mt-1.5"
                />
            ) : hasError ? (
...
function preprocessCitations(
    text: string,
    annotations: OpenSpecterCitationAnnotation[],
    citationsList: OpenSpecterCitationAnnotation[],
): string {
...
function MarkdownContent({
    ...
    text: string;
    citationsList: OpenSpecterCitationAnnotation[];
    onCitationClick?: (c: OpenSpecterCitationAnnotation) => void;
...
interface Props {
    ...
    isStreaming?: boolean;
    isError?: boolean;
    /** Human-readable error text rendered alongside the red Open Specter icon. */
    errorMessage?: string;
    annotations?: OpenSpecterCitationAnnotation[];
    onCitationClick?: (citation: OpenSpecterCitationAnnotation) => void;
    minHeight?: string;
    onWorkflowClick?: (workflowId: string) => void;
    onEditViewClick?: (ann: OpenSpecterEditAnnotation, filename: string) => void;
    /**
...
export function AssistantMessage({
    ...
    const citationsList: OpenSpecterCitationAnnotation[] = [];
        ...
        if (event.type === "tool_call_start") {
            return (
                ...
                >
                    ...
                    <ThinkingText testId="assistant-tool-call-thinking-indicator" />
                    <span className="ml-1 font-serif text-sm text-gray-500">
                        {event.name ? `Running ${event.name}...` : "Running tool..."}
                    </span>
                </div>
            );
        }
        if (event.type === "thinking") {
            return (
                ...
                >
                    ...
                    <ThinkingText testId="assistant-event-thinking-indicator" />
                </div>
            );
        }
    ...
    return (
        <div style={{ minHeight }}>
            ...
            <div className="w-full font-inter relative mt-2">
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        {!isStreaming &&
                            (() => {
                                ...
                                const statusOf = (ann: OpenSpecterEditAnnotation) =>
                ...
                {isError && (
                    <div className="mt-2 flex items-start gap-2 rounded-lg border border-border bg-muted/50 px-3 py-2 text-sm font-serif text-foreground">
