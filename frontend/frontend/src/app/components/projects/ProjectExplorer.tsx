...
import type { OpenSpecterDocument, OpenSpecterFolder } from "@/app/components/shared/types";
import { VersionChip } from "@/app/components/shared/VersionChip";

interface Props {
    projectName?: string | null;
    documents: OpenSpecterDocument[];
    folders?: OpenSpecterFolder[];
    selectedDocId?: string | null;
    onDocClick: (doc: OpenSpecterDocument) => void;
...
export function ProjectExplorer({
    ...
    async function handleDropOnTarget(targetFolderId: string | null, e: React.DragEvent) {
        ...
        const movingFolderId = e.dataTransfer.getData("application/open-specter-folder");
    ...
    function isInternalDrag(e: React.DragEvent): boolean {
        return (
            Array.from(e.dataTransfer.types).includes("application/open-specter-doc") ||
            Array.from(e.dataTransfer.types).includes("application/open-specter-folder")
        );
    }
                        ...
                        <li key={`f-${folder.id}`}>
                            <div
                                ...
                                className={`flex items-center gap-1.5 py-1.5 pr-2 rounded-sm cursor-pointer select-none transition-colors group ${
                                    isDragTarget
                                        ? "bg-muted/50 ring-1 ring-inset ring-ring/30"
                                        : "hover:bg-gray-50"
                                }`}
                                style={{ paddingLeft: basePadding }}
            ...
            {contextMenu && (
                ...
                >
                    ...
                    {contextMenu.folderId && onDeleteFolder && (
                        <button
                            className="flex w-full items-center gap-2 px-3 py-1.5 text-left text-foreground hover:bg-muted/50"
                            onClick={() => {
