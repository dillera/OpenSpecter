// Shared TypeScript types for Open Specter AI legal assistant

export interface OpenSpecterFolder {
  id: string;
  project_id: string;
  user_id: string;
  name: string;
...
export interface OpenSpecterProject {
  id: string;
  user_id: string;
  is_owner?: boolean;
  name: string;
...
export interface OpenSpecterDocument {
  id: string;
  user_id?: string;
...
export interface OpenSpecterChat {
  id: string;
  project_id: string | null;
  user_id: string;
  title: string | null;
...
export interface OpenSpecterEditAnnotation {
  type?: "edit_data";
  kind?: "edit";
  edit_id: string;
  document_id: string;
...
export type AssistantEvent =
  ...
  | {
        ...
        version_number?: number | null;
        download_url: string;
        annotations: OpenSpecterEditAnnotation[];
        error?: string;
        isStreaming?: boolean;
    }
  | { type: "content"; text: string; isStreaming?: boolean };

export interface OpenSpecterMessage {
  role: "user" | "assistant";
  content: string;
 ...
 */
export interface OpenSpecterCitationAnnotation {
  type: "citation_data";
  ref: number;
  doc_id: string;
  document_id: string;
...
export function expandCitationToEntries(
  a: OpenSpecterCitationAnnotation,
): CitationQuote[] {
  const rangeMatch =
...
export function formatCitationPage(a: OpenSpecterCitationAnnotation): string {
  if (typeof a.page === "string") return `Page ${a.page}`;
  return `Page ${a.page}`;
}

/** Produce a reader-friendly version of the quote (replaces [[PAGE_BREAK]] with "..."). */
export function displayCitationQuote(a: OpenSpecterCitationAnnotation): string {
...
export interface OpenSpecterWorkflow {
  id: string;
  user_id: string | null;
  title: string;
...
export interface OpenSpecterChatDetailOut {
  chat: OpenSpecterChat;
  messages: OpenSpecterMessage[];
}
